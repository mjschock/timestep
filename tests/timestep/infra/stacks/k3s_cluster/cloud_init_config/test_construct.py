...
from timestep.infra.stacks.k3s_cluster.cloud_init_config.construct import CloudInitConfigConstruct
...
class TestMain:
    ...
    def test_should_have_a_local_provider(self):
        assert Testing.to_have_provider(self.synthesized, LocalProvider.TF_RESOURCE_TYPE)

    def test_should_have_a_data_local_file(self):
        assert Testing.to_have_resource(self.synthesized, DataLocalFile.TF_RESOURCE_TYPE)
