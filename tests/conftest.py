...
async def client(current_loop, monkeypatch):
    ...
    base_url = settings.openai_base_url.replace(
        "/api/openai/v1", ""
    )  # TODO: create a new setting for the base_url
    ...
    monkeypatch.setenv(
        "OPENAI_BASE_URL",
        settings.openai_base_url.replace(base_url, "http://openai.local"),
    )
    assert (
        os.getenv("OPENAI_BASE_URL") == "http://openai.local/api/openai/v1"
    ), f"Unexpected OPENAI_BASE_URL: {os.getenv('OPENAI_BASE_URL')}"
