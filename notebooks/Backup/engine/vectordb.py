...
image_vector_store: PGVectorStore = None
...
def get_image_vector_store():
    global image_vector_store

    if image_vector_store is None:
        original_conn_string = os.environ.get("PG_CONNECTION_STRING")
        if original_conn_string is None or original_conn_string == "":
            raise ValueError("PG_CONNECTION_STRING environment variable is not set.")
        ...
        image_vector_store = PGVectorStore(
            connection_string=conn_string,
            async_connection_string=async_conn_string,
            schema_name=PGVECTOR_SCHEMA,
            # table_name=PGVECTOR_TABLE,
            table_name="llamaindex_image_embedding",
            embed_dim=int(os.environ.get("IMAGE_EMBEDDING_DIM", 512)),
        )
