...
class ResponseDAO:
    ...
    def get_response_by_id(self, response_id: str) -> ResponseTable | None:
        """Get a response by its ID."""
        statement = select(ResponseTable).where(ResponseTable.id == response_id)
        return self.session.exec(statement).first()

    def update_response(
        self, response_id: str, response_data: dict
    ) -> ResponseTable | None:
        ...
        for key, value in response_data.items():
            if hasattr(response, key):
                if key in ["tools", "output", "usage", "metadata"]:
                    ...
                    setattr(
                        response, key, value if isinstance(value, str) else str(value)
                    )
                else:
