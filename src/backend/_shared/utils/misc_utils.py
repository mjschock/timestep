...
# Test function with non-English examples
def test_language_agnostic_fallback():
    """Test with various languages to verify language-agnostic behavior."""

    tools = [
        {
            "function": {
                "name": "search_processor",
                "description": "Process search queries and return results",
                "parameters": {
                    "properties": {
        ...
        },
        {
            "function": {
                "name": "data_analyzer",
                "description": "Analyze and process various types of data",
                "parameters": {
                    "properties": {
                        "data_source": {
                            "type": "string",
    ...
    # Test cases in various languages and scripts
    test_cases = [
        'query "machine learning" max_results 10',  # English-like
        'recherche "apprentissage automatique" resultats 5',  # French-like
        "consulta datos analysis_depth 3",  # Spanish-like
        'データ分析 data_source "sales" format_output 1',  # Mixed Japanese
        'поиск query "информация" max_results 7',  # Mixed Cyrillic
        "分析数据 analysis_depth 2 format_output 0",  # Chinese with English
        " 42",
    ]

    print("=== Language-Agnostic Schema-Aware Tool Call Fallback ===\n")

    for i, response in enumerate(test_cases, 1):
        print(f"Test Case {i}: '{response}'")

        try:
            result = create_fallback_tool_call(response, tools)
            tool_name = result[0]["function"]["name"]
            args = json.loads(result[0]["function"]["arguments"])

            # Clean display
            display_args = {k: v for k, v in args.items() if not k.startswith("__")}
            confidence = args.get("__confidence_score", 0)
...
if __name__ == "__main__":
    test_language_agnostic_fallback()
