...
class ResponseTable(SQLModel, table=True):
    ...
    previous_response_id: str | None = Field(
        default=None, description="ID of previous response"
    )
    ...
    created: datetime = Field(
        default_factory=datetime.utcnow, description="Database creation timestamp"
    )
    updated: datetime = Field(
        default_factory=datetime.utcnow, description="Database update timestamp"
    )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "user": self.user,
            "metadata": json.loads(self.response_metadata)
