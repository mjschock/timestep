...
def _get_default_prefix() -> str:
    return """You are an AI who performs one task based on the following objective: {objective}. Take into account these previously completed tasks: {context}."""


def _get_default_suffix() -> str:
...
class LangChainBabyAGIAgent(Agent):
    @dataclass
    class Config:
        ...
        def __post_init__(self):
            ...
            if self.task_execution_chain is None:
                ...
                self.task_execution_chain = TaskExecutionChain.from_llm(
