...
def convert_mamba_ssm_checkpoint_to_huggingface_model(
    ...
    if not is_mamba_ssm_available():
        msg = "Calling convert_mamba_ssm_checkpoint_to_huggingface_model requires the mamba_ssm library to be installed. Please install it with `pip install mamba_ssm`."
        raise ImportError(
            msg
        )
...
def validate_converted_model(
    ...
    if not torch.allclose(original_model_logits, hf_model_logits, atol=1e-3):
        msg = "The converted model did not return the same logits as the original model."
...
def convert_mamba_checkpoint_file_to_huggingface_model_file(
    ...
    if not is_mamba_ssm_available():
        msg = "Calling convert_mamba_checkpoint_file_to_huggingface_model_file requires the mamba_ssm library to be installed. Please install it with `pip install mamba_ssm`."
        raise ImportError(
            msg
        )
    if not torch.cuda.is_available():
        msg = "This script is to be run with a CUDA device, as the original mamba_ssm model does not support cpu."
        raise ValueError(
            msg
        )
    ...
    with open(config_json_file, encoding="utf-8") as json_file:
