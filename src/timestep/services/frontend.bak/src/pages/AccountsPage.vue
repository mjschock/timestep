  ...
  <q-card-actions vertical v-if="selectedAccountTypeIsPrimary">
    <q-form @submit="changePassword">
      <q-card-actions vertical>
        ...
        <q-btn color="primary" label="Change password" no-caps type="submit" />
      </q-card-actions>
    </q-form>
...
export default defineComponent({
  ...
  setup() {
    ...
    const selectedAccountTypeIsPrimary = computed(() => {
      return (
        selected.value.length === 1 && selected.value[0].type === 'Primary'
      );
    });
    const totp = ref({
      imageUrl: '',
    ...
    const changePassword = async (e: Event) => {
      ...
      const resp: ChangePasswordResponse = await nhost.auth.changePassword(
        params
      );
    ...
    const deleteAccount = async (e: Event) => {
      ...
      const { res, error } = await nhost.functions.call<
        ...
        Body,
        ErrorMessage
      >('accounts', { id: selected.value[0].id });
    ...
    return {
      ...
      getSelectedString() {
        ...
        return selected.value.length === 0
          ? ''
          : `${selected.value.length} account${
              selected.value.length > 1 ? 's' : ''
            } selected of ${accounts.value.length}`;
      },
      isPwd,
      password,
      prompt,
