...
def env(render_mode=None):
    ...
    if isinstance(env.gym_env.action_space, Discrete):
...
class raw_env(AECEnv):
    ...
    # metadata = {"render_modes": ["human"], "name": "lunar_lander_v2", "is_parallelizable": False}
    metadata = {"name": "mountain_car_continuous_v0", "is_parallelizable": False}

    def __init__(self, render_mode=None, **kwargs: Any):
        """
        ...
        self.gym_env: gym.Env = gym.make(  # TODO: name this env?
            id="MountainCarContinuous-v0", # TODO: use EnvSpec?
            render_mode=render_mode,
            **kwargs,
        )

        # self.metadata["render_modes"] = self.gym_env.metadata["render_modes"]
        self.metadata.update(self.gym_env.metadata)
        ...
        self._action_spaces: dict[str, gym.Space[ActType]] = {agent: self.gym_env.action_space for agent in self.possible_agents}
    ...
    def reset(self, seed=None, options=None):
        ...
        observation, info = self.gym_env.reset(seed=seed, options=options)
