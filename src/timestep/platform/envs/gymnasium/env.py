...
class RawEnv(AECEnv):
    ...
    def __init__(self, id, name, render_mode=None, **kwargs: Any):
        ...
        # self.gym_env: gym.Env = gym.make(  # TODO: name this env?
        self.env: gym.Env = gym.make(
            # id=id, # TODO: use EnvSpec?
            id=self.id,
        ...
        #     print(f"{self.id} is a GoalEnv")
        #     print('self.gym_env.observation_space: ', self.gym_env.observation_space)

        #     for key in ["observation", "achieved_goal", "desired_goal"]:
        #         if key not in self.gym_env.observation_space.spaces:
        #             raise ValueError(
        #                 f"GoalEnv must have observation_space with key '{key}'"
        #             )

        #     print('self.gym_env.observation_space.spaces["observation"]')
        #     print(self.gym_env.observation_space.spaces["observation"])
        ...
        self._action_spaces: dict[str, gym.Space[ActType]] = {
            agent: self.env.action_space for agent in self.possible_agents
        }
        self._observation_spaces: dict[str, gym.Space[ObsType]] = {
    ...
    def step(self, action):
        ...
        # self.state[self.agent_selection] = action # TODO: why is this self.state?
        ...
        assert self._agent_selector.is_last(), "Only one agent is currently supported"
...
def env_creator(id, name, render_mode=None):
    """
    ...
    # class RawEnv(GenericRawEnv):
    #     id = id
    #     metadata = {"is_parallelizable": False, "name": name}

    # env = raw_env(render_mode=internal_render_mode)
    env = RawEnv(id, name, render_mode=internal_render_mode)

    # This wrapper is only for environments which print results to the terminal
    if render_mode == "ansi":
        env = wrappers.CaptureStdoutWrapper(env)

    if isinstance(env.env.action_space, Discrete):
...
def envy(__name__, namespace=None):
    ...
    id = f"{''.join(capitalized_words_in_name_without_version)}-{version}"
