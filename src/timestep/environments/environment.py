...
class Environment(
    ...
    def __init__(
        ...
        self.metadata = { "is_parallelizable": True, "name": f"{env_name}", "render_modes": ["ansi", "human", "rgb_array", "svg"] }
    ...
    def observe(self, agent: AgentID) -> ObsType | None:
        ...
        assert isinstance(observation, np.ndarray), f"Observation must be a numpy array, got {type(observation)}."
    ...
    def reset(
        ...
        self.action_spaces = { f"{env_name}": gymnasium.spaces.Box(low=0.0, high=255.0, shape=box_shape, dtype=np.uint8, seed=seed) }
        ...
        self.observation_spaces = { f"{env_name}": gymnasium.spaces.Box(low=0.0, high=255.0, shape=box_shape, dtype=np.uint8, seed=seed) }
