...
class KubeConfigConstruct(Construct):
    def __init__(
        ...
        local_exec_provisioner = LocalExecProvisioner(
            command=f"ls -al ~ && ls -al {ssh_private_key_path} && k3sup install --context {kubecontext} --ip {ipv4} --k3s-extra-args '--disable traefik' --local-path {local_path} --skip-install --ssh-key {ssh_private_key_path} --user {username}",  # noqa: E501
            type="local-exec",
        )
