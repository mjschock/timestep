        ...
        kube_config_resource = Resource(
            ...
            provisioners=[
                LocalExecProvisioner(
                    command=f"k3sup install --context {kubecontext} --ip {ipv4} --local-path {local_kube_config_path} --merge --skip-install --ssh-key {ssh_private_key_path} --user {username}",  # noqa: E501
